...
export const alt = 'SkillLedger Category'
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = category?.name ?? 'Skill Category'
  const skillCount = category?.sampleSkills.length ?? 0
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ fontSize: 20, color: '#94a3b8', display: 'flex' }}>
            {skillCount} skills available for exchange
          </div>
        </div>
